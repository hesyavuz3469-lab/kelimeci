      ...
      if (key === "⌫" || key === "BACKSPACE") {
        setRows((prev) => {
          const next = prev.map((r, i) => {
            ...
            const lastFilled = letters.map((l) => l !== "").lastIndexOf(true);
            if (lastFilled >= 0) letters[lastFilled] = "";
            return { ...r, letters };
          });
          saveGameState({
            rows: next,
      ...
      if (key === "ENTER") {
        setRows((prev) => {
          ...
          const newRows = prev.map((r, i) => {
            if (i !== currentRow) return r;
            return { ...r, states, submitted: true };
          });
          ...
          if (newStatus !== "playing") {
            setScore(finalScore);
            if (timerRef.current) clearInterval(timerRef.current);
            setTimeout(() => setShowResult(true), won ? 1600 : 800);
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex flex-col">
      ...
      <header className="border-b border-zinc-800 px-4 py-3 flex items-center justify-between">
        ...
        <div className="text-zinc-400 text-sm font-mono min-w-[40px] text-right">
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 bg-white text-black font-bold px-5 py-2.5 rounded-xl z-40 shadow-lg text-sm animate-fade-in">
          {toast}
        </div>
      )}

      {/* Game area */}
      <main className="flex-1 flex flex-col items-center justify-center gap-6 px-4 py-4">
        <Board
          rows={rows}
